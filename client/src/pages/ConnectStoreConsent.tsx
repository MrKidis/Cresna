...
export default function ConnectStoreConsent() {
  ...
  return (
    <WorkspaceFrame eyebrow="Step 1 of 3 · Connect" title="Make an informed data connection." description="Cresna will not start Shopify authorization until you review and actively confirm every requested read scope below.">
      <section className="mb-5 rounded-[1.1rem] border border-border bg-secondary p-4"><div className="grid gap-3 text-xs font-bold sm:grid-cols-3"><span className="flex items-center gap-2 text-secondary-foreground"><CircleCheck className="h-4 w-4 text-primary" />1. Review and connect</span><Button type="button" variant="ghost" onClick={() => setLocation("/app/profile")} className="h-auto justify-start gap-2 px-0 py-0 text-left text-muted-foreground hover:bg-transparent hover:text-foreground"><span className="grid h-4 w-4 place-items-center rounded-full border border-current text-[9px]">2</span>Tell Cresna about your goals</Button><span className="flex items-center gap-2 text-muted-foreground"><span className="grid h-4 w-4 place-items-center rounded-full border border-current text-[9px]">3</span>Review Growth Profile</span></div></section>

      <section className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
        <div className="rounded-[1.35rem] border border-border bg-card p-6 text-card-foreground sm:p-8">
          ...
          <div className="mt-7 space-y-3">
            {dataPermissions.map(permission => <label key={permission.key} className={`flex cursor-pointer gap-4 rounded-2xl border p-4 transition-colors ${accepted[permission.key] ? "border-primary/35 bg-secondary" : "border-border bg-background hover:border-primary/30"}`}><input type="checkbox" checked={accepted[permission.key]} onChange={event => setAccepted(previous => ({ ...previous, [permission.key]: event.target.checked }))} className="mt-0.5 h-4 w-4 shrink-0 accent-primary" aria-describedby={`${permission.key}-detail`} /><span><span className="flex flex-wrap items-center gap-2"><span className="text-sm font-bold text-foreground">{permission.title}</span><code className="rounded bg-muted px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">{permission.scope}</code></span><span id={`${permission.key}-detail`} className="mt-1.5 block text-xs leading-5 text-muted-foreground"><strong className="font-semibold text-foreground">Cresna reads:</strong> {permission.reads}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground"><strong className="font-semibold text-foreground">Why:</strong> {permission.purpose}</span></span></label>)}
          </div>
          ...
          <form className="mt-8 max-w-xl space-y-3" onSubmit={event => { event.preventDefault(); if (!allConsentAccepted) return; beginAuthorization.mutate({ shopDomain }); }}>
            <Label htmlFor="shop-domain" className="text-xs font-bold text-foreground">Store domain</Label>
            <Input id="shop-domain" value={shopDomain} onChange={event => setShopDomain(event.target.value)} placeholder="your-store.myshopify.com" className="h-12 rounded-xl border-input bg-background text-foreground" required />
            <Button type="submit" disabled={beginAuthorization.isPending || !allConsentAccepted} className="mt-3 h-12 w-full rounded-full text-xs font-bold disabled:cursor-not-allowed"><img src="/manus-storage/shopify-primary-logo_3d6b973f.svg" alt="Shopify" className="h-5 w-auto rounded-sm bg-white px-1.5 py-0.5" />{beginAuthorization.isPending ? "Opening Shopify…" : "Continue to Shopify"}</Button>
          ...
          <Button type="button" variant="link" onClick={() => sync.mutate()} disabled={sync.isPending} className="mt-5 h-auto px-0 text-[11px] font-bold text-muted-foreground hover:text-foreground">{sync.isPending ? "Refreshing store scan…" : "Refresh an already connected store"}</Button>
        </div>
