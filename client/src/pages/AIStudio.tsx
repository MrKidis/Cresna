...
import { AIDraftEvidence } from "@/components/AIDraftEvidence";
import { AIDraftStatusSummary } from "@/components/AIDraftStatusSummary";
import { MerchantAILoadingSummary } from "@/components/MerchantAILoadingSummary";
import { MerchantDraftReviewCard } from "@/components/MerchantDraftReviewCard";
...
export default function AIStudio() {
  ...
  return <WorkspaceFrame eyebrow="AI Action Studio" title="Turn a signal into merchant-approved work." description="Choose a product, generate a draft that preserves your approved context, then decide whether Cresna should remember it as a direction. Nothing is sent to Shopify automatically.">
    <MerchantAILoadingSummary isLoading={aiIsWorking} />
    {!catalog?.length ? <EmptyWorkspaceCard title="Connect and scan a Shopify catalog first" action="Connect store" onAction={() => window.location.assign("/app/connect")}>Cresna only creates custom drafts from products it can verify in your connected catalog.</EmptyWorkspaceCard> : <><div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
      <section className="rounded-[1.35rem] border border-border bg-card p-6 text-card-foreground shadow-[0_8px_30px_rgba(23,32,30,0.04)] sm:p-7">
        <div className="flex items-start gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-brand-accent text-[#17201e]"><Sparkles className="h-4 w-4" /></span><div><p className="eyebrow text-[10px] text-muted-foreground">Custom action · Product content</p><h2 className="mt-1 text-xl font-extrabold tracking-[-0.05em]">Create a better product draft.</h2></div></div>
        ...
        <Button onClick={() => selectedProductId && generate.mutate({ productId: selectedProductId, recommendationId })} disabled={!selectedProductId || aiIsWorking} className="mt-6 h-11 w-full rounded-full bg-brand-surface text-xs font-bold text-brand-surface-foreground hover:bg-[#293630]">{generate.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Reading product facts & drafting…</> : <><Sparkles className="mr-2 h-4 w-4" />Generate product draft</>}</Button>
      ...
      <section className="rounded-[1.35rem] border border-[#17201e]/12 bg-brand-surface p-6 text-brand-surface-foreground sm:p-7"><div className="flex items-center justify-between gap-4"><div><p className="eyebrow text-[10px] text-brand-muted-foreground">Review queue</p><h2 className="mt-1 text-xl font-extrabold tracking-[-0.05em]">Your AI work stays yours.</h2></div><span className="rounded-full bg-brand-accent px-3 py-1 text-[10px] font-extrabold text-[#17201e]">{drafts?.length || 0} drafts</span></div>{draftsLoading ? <div className="grid min-h-[250px] place-items-center"><Loader2 className="h-5 w-5 animate-spin" /></div> : !drafts?.length ? <div className="mt-6 grid min-h-[300px] place-items-center rounded-xl border border-dashed border-white/20 px-7 text-center"><div><FileText className="mx-auto h-6 w-6 text-brand-accent" /><p className="mt-4 text-sm font-bold">No custom drafts yet.</p><p className="mt-2 text-xs leading-5 text-brand-muted-foreground">Create a product or positioning draft from evidence Cresna can verify. It will appear here for review.</p></div></div> : <div className="mt-6 space-y-4">{drafts.slice(0, 4).map(({ draft, product, parsedDraft }) => <article key={draft.id} className="rounded-xl border border-white/15 bg-white/[0.05] p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold">{draft.actionType === "positioning" ? "Brand positioning" : product?.title || "Catalog item"}</p><p className="mt-1 text-[11px] text-brand-muted-foreground">{draft.status === "generated" ? "Awaiting your decision" : draft.status === "approved" ? "Approved — remembered by Cresna" : "Rejected — decision recorded"}</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${draft.status === "approved" ? "bg-brand-accent text-[#17201e]" : draft.status === "rejected" ? "bg-white/15 text-brand-surface-foreground" : "bg-white/10 text-brand-surface-foreground"}`}>{draft.status}</span></div><div className="mt-4 max-h-36 overflow-auto rounded-lg bg-black/15 p-3 font-mono text-[11px] leading-5 text-[#e9eee9]">{parsedDraft?.descriptionHtml || parsedDraft?.positioning || draft.generatedContent}</div>{parsedDraft ? <MerchantDraftReviewCard draft={parsedDraft} /> : null}{draft.status === "generated" ? <div className="mt-4 flex flex-wrap gap-2"><Button size="sm" onClick={() => review.mutate({ draftId: draft.id, status: "approved" })} disabled={review.isPending} className="h-8 rounded-full bg-brand-accent px-3 text-[11px] font-bold text-[#17201e] hover:bg-brand-accent"><Check className="mr-1 h-3.5 w-3.5" />Approve direction</Button><Button size="sm" variant="outline" onClick={() => review.mutate({ draftId: draft.id, status: "rejected" })} disabled={review.isPending} className="h-8 rounded-full border-white/25 bg-transparent px-3 text-[11px] text-brand-surface-foreground hover:bg-white/10 hover:text-brand-surface-foreground"><X className="mr-1 h-3.5 w-3.5" />Reject</Button><Button size="sm" variant="ghost" onClick={() => navigator.clipboard.writeText(parsedDraft?.descriptionHtml || parsedDraft?.positioning || draft.generatedContent).then(() => toast.success("Draft copied"))} className="h-8 rounded-full px-3 text-[11px] text-brand-surface-foreground hover:bg-white/10 hover:text-brand-surface-foreground"><Copy className="mr-1 h-3.5 w-3.5" />Copy</Button></div> : null}</article>)}</div>}</section>
    </div><AIDraftEvidence drafts={drafts ?? []} /><MerchantWriteApprovalPanel drafts={drafts ?? []} approvals={writeApprovals ?? []} pending={requestWriteApproval.isPending} onRequest={draftId => requestWriteApproval.mutate({ draftId })} /></>}
  </WorkspaceFrame>;
}
