...
export default function AIStudio() {
  ...
  return <WorkspaceFrame eyebrow="AI Action Studio" title="Turn a signal into merchant-approved work." description="Choose a product, generate a draft that preserves your approved context, then decide whether Cresna should remember it as a direction. Nothing is sent to Shopify automatically.">
    {!catalog?.length ? <EmptyWorkspaceCard title="Connect and scan a Shopify catalog first" action="Connect store" onAction={() => window.location.assign("/app/connect")}>Cresna only creates custom drafts from products it can verify in your connected catalog.</EmptyWorkspaceCard> : <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
      <section className="rounded-[1.35rem] border border-[#17201e]/12 bg-[#fdfdfb] p-6 shadow-[0_8px_30px_rgba(23,32,30,0.04)] sm:p-7">
        <div className="flex items-start gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#d9fa55] text-[#17201e]"><Sparkles className="h-4 w-4" /></span><div><p className="eyebrow text-[10px] text-[#65706b]">Custom action · Product content</p><h2 className="mt-1 text-xl font-extrabold tracking-[-0.05em]">Create a better product draft.</h2></div></div>
        <label className="mt-7 grid gap-2 text-xs font-bold text-[#17201e]">Choose a connected product<select value={selectedProductId || ""} onChange={event => setSelectedProductId(Number(event.target.value))} className="h-11 rounded-xl border border-[#17201e]/15 bg-[#f5f5f1] px-3 text-sm font-medium text-[#17201e] focus:outline-none focus:ring-2 focus:ring-[#7c9b1e]/45">{catalog.map(product => <option key={product.id} value={product.id}>{product.title}</option>)}</select></label>
        {selectedProduct ? <div className="mt-5 rounded-xl border border-[#17201e]/10 bg-[#f5f5f1] p-4"><p className="text-sm font-bold">{selectedProduct.title}</p><div className="mt-3 grid grid-cols-3 gap-3 text-xs"><div><p className="text-[#65706b]">Description</p><p className="mt-1 font-bold">{textOnly(selectedProduct.descriptionHtml || "").length >= 80 ? "Present" : "Thin or missing"}</p></div><div><p className="text-[#65706b]">SEO</p><p className="mt-1 font-bold">{selectedProduct.seoTitle && selectedProduct.seoDescription ? "Present" : "Incomplete"}</p></div><div><p className="text-[#65706b]">Media</p><p className="mt-1 font-bold">{selectedProduct.mediaCount || 0} assets</p></div></div></div> : null}
        <div className="mt-5 rounded-xl border border-[#17201e]/10 p-4"><p className="text-xs font-bold">Business Brain context used</p><p className="mt-2 text-xs leading-5 text-[#65706b]">{intelligence?.profile?.positioning || intelligence?.profile?.brandSummary || "Add your positioning and brand summary in Growth Profile to make every draft more specific."}</p></div>
        {recommendationId ? <p className="mt-4 rounded-lg bg-[#f0f5d8] px-3 py-2 text-[11px] font-semibold text-[#526400]">This draft will be linked to your approved Opportunity Engine item.</p> : null}
        <Button onClick={() => selectedProductId && generate.mutate({ productId: selectedProductId, recommendationId })} disabled={!selectedProductId || generate.isPending} className="mt-6 h-11 w-full rounded-full bg-[#17201e] text-xs font-bold text-[#f8f7f2] hover:bg-[#293630]">{generate.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating your draft…</> : <><Sparkles className="mr-2 h-4 w-4" />Generate product draft</>}</Button>
        <Button onClick={() => generatePositioning.mutate()} disabled={generatePositioning.isPending} variant="outline" className="mt-3 h-10 w-full rounded-full border-[#17201e]/20 bg-transparent text-xs font-bold text-[#17201e] hover:bg-[#f5f5f1]">{generatePositioning.isPending ? <><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" />Creating positioning…</> : <><Sparkles className="mr-2 h-3.5 w-3.5" />Generate positioning draft</>}</Button>
        <p className="mt-4 text-center text-[11px] leading-5 text-[#65706b]">Cresna will not claim facts that are absent from your product record. You review every draft before a future publishing step.</p>
      </section>
