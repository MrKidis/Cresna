...
import { products, productDailyMetrics, storeDailyMetrics, stores } from "../drizzle/schema";
...
export async function syncShopifyStore(storeId: number) {
  ...
  const [orderResult, productResult] = await Promise.all([
    ...
    shopifyGraphql<{ products: { nodes: Array<{ id: string; title: string; handle?: string | null; vendor?: string | null; productType?: string | null; status: string; totalInventory?: number | null; updatedAt: string }> } }>(store.myshopifyDomain, accessToken, "query Products { products(first: 250) { nodes { id title handle vendor productType status totalInventory updatedAt } } }"),
  ]);
  ...
  const now = new Date();
  for (const product of productResult.products.nodes) await db.insert(products).values({ storeId: store.id, shopifyProductId: product.id, title: product.title, handle: product.handle || null, vendor: product.vendor || null, productType: product.productType || null, status: product.status, totalInventory: product.totalInventory ?? null, updatedAtSource: new Date(product.updatedAt) }).onDuplicateKeyUpdate({ set: { title: product.title, handle: product.handle || null, vendor: product.vendor || null, productType: product.productType || null, status: product.status, totalInventory: product.totalInventory ?? null, updatedAtSource: new Date(product.updatedAt), updatedAt: now } });
  ...
  await setGrowthProfileScanStatus(store.userId, daily.size >= 7 ? "ready" : "needs_more_data");
  await refreshRevenueImpactForStore(store.id);
  return { daysSynced: daily.size, productsSynced: productResult.products.nodes.length };
}
